...
import static AsyncMongoClients.getDefaultCodecRegistry
...
class AsyncMongoClientSpecification extends Specification {
    ...
    def 'should use ListDatabasesIterableImpl correctly'() {
        ...
        then:
        expect listDatabasesIterable, isTheSameAs(new AsyncListDatabasesIterableImpl<Document>(session, Document, getDefaultCodecRegistry(),
        ...
        then:
        expect listDatabasesIterable, isTheSameAs(new AsyncListDatabasesIterableImpl<BsonDocument>(session, BsonDocument,
        ...
        expect listDatabaseNamesIterable.getMapped(), isTheSameAs(new AsyncListDatabasesIterableImpl<BsonDocument>(session, BsonDocument,
    ...
    def 'should pass the correct settings to getDatabase'() {
        ...
        def settings = MongoClientSettings.builder()
                                          ...
                                          .build()
        def client = new AsyncMongoClientImpl(settings, Stub(Cluster), new TestOperationExecutor([]))
        ...
        where:
        expectedDatabase << new AsyncMongoDatabaseImpl('name',
    ...
    def 'should cleanly close the external resource closer on close'() {
        given:
        def closed = false
        def client = new AsyncMongoClientImpl(MongoClientSettings.builder().build(), Mock(Cluster), {
            closed = true
